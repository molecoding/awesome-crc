...
public class ModbusCrc {
  private static final byte[] highOrderDataTable = {0x00, (byte) 0xC1, (byte) 0x81,
  ...
  private static final byte[] lowOrderDataTable = {(byte) 0x00, (byte) 0xC0, (byte) 0xC1,
  ...
  public static int gen(byte[] bytes) {
    byte ho = (byte) 0xff;
    byte lo = (byte) 0xff;
    int idx;
    for (byte b : bytes) {
      idx = (ho ^ b) & 0xff;
      ho = (byte) (lo ^ highOrderDataTable[idx]);
      lo = lowOrderDataTable[idx];
    }
    return ((((int) ho) << 8 | (((int) lo) & 0xff))) & 0xffff;
  }
